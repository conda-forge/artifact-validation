...
if hexversion >= 0x03070000 or (
    implementation.name == "cpython" and hexversion >= 0x03060000
):
    ordered_dict = dict
else:
...
escaped_glob_replacement = re.compile(
    "(%s)" % "|".join(escaped_glob_tokens_to_re).replace("\\", "\\\\\\")
)


def glob_to_re(pattern):
    return escaped_glob_replacement.sub(
        lambda match: escaped_glob_tokens_to_re[match.group(0)], re.escape(pattern)
    )
